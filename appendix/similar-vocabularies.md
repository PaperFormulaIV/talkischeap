...
| 协变量 | 协变量偏移 | 量纲 | 无量纲化 | 数据集 | 输入 | 输出 | 样本 | 特征 |特征图 | 批量标准化 | 标准化 | 归一化 | 中心化 | 标准化、归一化、中心化的区别 | 过拟合 | 欠拟合 | 特征缩放 | 损失函数 | 准确率 | 随机梯度下降 | 有监督学习 | 无监督学习 | 半监督学习 | 向前传播 | 向后传播 |
...
- 随机梯度下降（random / stochastic gradient descent）「详见[字母索引R](//todo)」
